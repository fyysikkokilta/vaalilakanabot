...
async def add_selected_tag(
    ...
    try:
        ...
        text = parse_command_parameters(update.message.text, "/selected")
        params = text.split(",")
...
async def edit_or_add_new_role(
    ...
    try:
        ...
        text = parse_command_parameters(update.message.text, "/edit_or_add_new_role")
...
async def remove_role(update: Update, context: ContextTypes.DEFAULT_TYPE, data_manager):
    ...
    try:
        ...
        text = parse_command_parameters(update.message.text, "/remove_role")
        params = text.split(",")
...
async def export_data(update: Update, context: ContextTypes.DEFAULT_TYPE, data_manager):
    ...
    try:
        ...
        text = parse_command_parameters(update.message.text, "/export_data")

        output = StringIO()
        output.write("Name,Role,Email,Telegram\n")

        if len(text) > 0:
            position_param = text.strip()
            # Find position by Finnish or English name
            found_position = data_manager.find_position_by_name(position_param)
            if not found_position:
                await update.message.reply_text(f"Unknown position: {position_param}")
                return
